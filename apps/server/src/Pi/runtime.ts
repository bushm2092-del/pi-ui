...
export type { AgentSession, AgentSessionRuntime } from "@earendil-works/pi-coding-agent";
...
export async function createPiRuntime(options: CreatePiRuntimeOptions): Promise<AgentSessionRuntime> {
  ...
  const sessionManager = options.sessionFile ? SessionManager.open(options.sessionFile) : SessionManager.create(options.cwd);
...
export async function bindHeadlessExtensions(session: AgentSession, onError: (error: PiExtensionErrorDto) => void): Promise<void> {
  await session.bindExtensions({
    mode: "json",
    onError(error) {
      onError({ extensionPath: error.extensionPath, event: error.event, message: error.error, stack: error.stack });
    },
  });
}
