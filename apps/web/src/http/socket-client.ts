...
import type { AgentBackendConnection, JsonValue, ResultVO, RuntimeSnapshotDto } from "@pi/shared";

export interface SocketClientOptions extends AgentBackendConnection {
...
/**
 * Low-level message sender. Owns the socket and exposes a single request
 * pipeline (connect, timeout, ResultVO → value/error) plus subscription
 * support. Business APIs import this instead of touching the socket.
 */
export class SocketClient {
    ...
    return {
      ...
      unsubscribe: () => {
        ...
        if (this.#socket.connected) {
          void this.request<null>("runtime:unwatch", { runtimeSlotId }).catch(() => undefined);
        }
      },
    };
  }

  async request<T>(event: string, payload: unknown, timeoutMs = 30_000): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      if (timeoutMs > 0) {
        this.#socket.timeout(timeoutMs).emit(event, payload, (error: Error | null, result: ResultVO<T>) => {
  ...
  async #restoreSubscriptions(): Promise<void> {
    await Promise.all(
      [...this.#subscriptions.entries()].map(async ([runtimeSlotId, listeners]) => {
        try {
          const snapshot = await this.request<RuntimeSnapshotDto>("runtime:watch", { runtimeSlotId });
  ...
  async #watch(runtimeSlotId: string, listener: RuntimeSubscriptionListener, reason: "synchronized" | "reset"): Promise<void> {
    try {
      listener.onSnapshot?.(await this.request<RuntimeSnapshotDto>("runtime:watch", { runtimeSlotId }), reason);
    } catch (error) {
