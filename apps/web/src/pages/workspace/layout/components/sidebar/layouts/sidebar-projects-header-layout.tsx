...
interface SidebarProjectsHeaderLayoutProps {
  label: string;
  expanded: boolean;
  onToggle: () => void;
}

export function SidebarProjectsHeaderLayout({
  label,
  expanded,
  onToggle,
}: SidebarProjectsHeaderLayoutProps) {
        ...
        <div {...({"className":"flex min-w-0 flex-1"} as any)}>
          ...
          >
            ...
            <svg
              ...
              aria-hidden="true"
              className={`icon-2xs shrink-0 transition-transform group-hover/section-toggle:opacity-100 group-focus-visible/section-toggle:opacity-100 sidebar-hover-icon-tint ${expanded ? "rotate-0 opacity-0" : "-rotate-90 opacity-100"}`}
            >
