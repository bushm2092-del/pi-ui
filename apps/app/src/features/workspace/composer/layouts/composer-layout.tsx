...
export function ComposerLayout({ slots = {}, rootProps = {} }: LayoutProps = {}) {
  return (
    <div {...({"className":"min-w-0","data-codex-composer-root":""} as any)} {...rootProps}>
      ...
      <div {...({"className":"flex w-full flex-col gap-2 relative"} as any)}>
        <div {...({"className":"relative"} as any)}>
          <div {...({"className":"relative flex flex-col composer-surface-chrome bg-token-input-background/90 backdrop-blur-lg electron:dark:bg-token-dropdown-background overflow-y-auto _multilineSurface_1xj1z_2"} as any)}>
            <div {...({"className":"relative z-10 flex min-h-0 flex-1 flex-col"} as any)}>
              <div {...({"className":"contents"} as any)}>
                <div {...({"className":"_attachmentsDefault_1xj1z_2"} as any)} />
              </div>
              <div {...({"className":"contents"} as any)}>
                <div {...({"className":"_footer_1xj1z_2 grid grid-cols-[minmax(0,auto)_auto_minmax(0,1fr)] items-center gap-x-[5px] select-none mb-2 px-2"} as any)}>
                        ...
                        <div {...({"className":"flex min-w-0 items-center gap-1"} as any)}>
                          <span>
                            <span {...({"data-state":"closed","type":"button","id":"radix-_r_l9_","aria-haspopup":"menu","aria-expanded":"false","className":"contents outline-hidden cursor-interaction"} as any)}>
                              <LayoutSlot name="composer-reasoning" slots={slots} />
                            </span>
                          </span>
                        </div>
                      </div>
                      <div {...({"className":"flex shrink-0 items-center gap-2"} as any)}>
                        <LayoutSlot name="composer-send" slots={slots} />
                      </div>
                    </div>
                  </div>
                </div>
