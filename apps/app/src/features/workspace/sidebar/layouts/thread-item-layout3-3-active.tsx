...
export function ThreadItemLayout3_3_Active({ slots = {}, rootProps = {} }: LayoutProps = {}) {
  return (
    <div {...({"className":"after:block after:h-px after:content-[''] last:after:hidden","role":"listitem","style":{"transition":"transform linear"}} as any)} {...rootProps}>
      <div {...({"className":"cursor-grab active:cursor-grabbing","role":"button","tabIndex":"0","aria-disabled":"false","aria-roledescription":"sortable","aria-describedby":"DndDescribedBy-3"} as any)}>
        <div {...({"className":"overflow-hidden","style":{"height":"auto","opacity":"1","overflow":"visible"}} as any)}>
          <div {...({"data-app-action-sidebar-thread-active":"true","data-app-action-sidebar-thread-host-id":"local","data-app-action-sidebar-thread-id":"local:019fb282-50a6-7930-8a59-b34b0d18f8af","data-app-action-sidebar-thread-kind":"local","data-app-action-sidebar-thread-pinned":"false","data-app-action-sidebar-thread-row":"","data-app-action-sidebar-thread-title":"确认启动桌宠是否持久化","className":"group relative cursor-interaction py-row-y text-sm hover:bg-token-list-hover-background focus-visible:outline-offset-[-2px] h-[var(--height-token-row)] sidebar-item pe-1 ps-[var(--padding-row-cell-x,var(--padding-row-x))] bg-token-list-hover-background","role":"button","tabIndex":"0","aria-label":"确认启动桌宠是否持久化","data-state":"closed","aria-current":"page"} as any)}>
            ...
            <div {...({"className":"flex h-full w-full items-center text-sm leading-4"} as any)}>
              <div {...({"className":"flex min-w-0 flex-1 items-center gap-2"} as any)}>
                ...
                <div {...({"className":"flex min-w-0 flex-1 self-stretch text-base leading-5 items-center gap-2 text-token-foreground","data-thread-title-trigger":"true"} as any)}>
                  <span {...({"data-thread-title":"true","id":"_r_gl_","className":"_viewport_1ozkg_1 _animateOnGroupHover_1ozkg_53 min-w-0 select-none flex-1","data-marquee-text":"true","draggable":"false"} as any)}>
