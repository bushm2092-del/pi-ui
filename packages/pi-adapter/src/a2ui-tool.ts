...
import type { A2uiToolDetails, JsonValue } from "@pi/protocol";
...
export const renderA2uiTool = defineTool({
  ...
  parameters: Type.Object({
    ...
    messages: Type.Array(Message, { minItems: 1 }),
  }),
  execute: async (_toolCallId, params) => {
    validateMessages(params.surfaceId, params.messages);
...
function validateMessages(surfaceId: string, messages: Record<string, unknown>[]): void {
  let hasChart = false;
  for (const message of messages) {
    if (message.version !== "v0.9" && message.version !== "v0.9.1") {
      throw new Error("Only A2UI v0.9/v0.9.1 messages are supported");
    }
    ...
    if (Array.isArray(components)) {
      hasChart ||= components.some((component) =>
