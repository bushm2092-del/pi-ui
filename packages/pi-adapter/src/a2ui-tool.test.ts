...
describe("renderA2uiTool", () => {
  ...
  it("fails when messages target another surface", async () => {
    await expect(renderA2uiTool.execute("call-1", {
      surfaceId: "sales",
      messages: [{
        version: "v0.9",
        updateComponents: { surfaceId: "other", components: [] },
      }],
    }, undefined, undefined, {} as never)).rejects.toThrow("surfaceId sales");
  });
});
